  ...
  return (
    ...
    <div className="relative w-full">
      ...
      <input
        ...
        value={transcript}
        id="voice-search"
        className="search-bar transition-all duration-300 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        placeholder="Search"
        onChange={(e) => setTranscript(e.target.value)}
        required
      />
      {transcript.length > 0 && (
        <button
          type="button"
          className="absolute inset-y-0 end-10 flex items-center ps-3"
          onClick={() => setTranscript("")}
        >
          <ClearIcon className="text-gray-500 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white" />
        </button>
      ...
      <button
        type="button"
        className={`absolute inset-y-0 end-0 flex items-center pe-3 ${isListening ? "text-blue-500" : "text-gray-500 dark:text-gray-400"} hover:text-gray-900 dark:hover:text-white`}
        onClick={startListening}
        disabled={isListening}
      >
        <svg
